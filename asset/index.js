...
if (lib.config['extension_忽悠宇宙_type'] == '0') {//按圆梦时间分类
	for (let data in allCharacter) {//键：日期
		...
		for (let name in allCharacter[data]) {//键：角色 or 技能名 or 技能名_info
			let values = allCharacter[data][name];//值：角色前置数组['',[],'',''] or 技能内容{} or 技能翻译'xx|xxxx'
			if (typeof values == 'object' && Array.isArray(values)) {//角色前置数组
				//武将标准数组
				characters[name] = values.find(i => Array.isArray(i));
				characterSorts[sortName].add(name)//加入扩展包
				//剩余包含三元素的数组
				...
				if (temps[1]) {//称号
					characterTitles[name] = '#g' + temps[1];
				}
				if (temps[2]) {//介绍
				...
				if (name.endsWith('_info') && values.includes('|')) {//'xxx_info':'xx|xxxx'
	...
	game.import("character", () => {
		...
		return {
			...
			characterSubstitute: {
				meng_furina: [
					["mengjvxing_achieve", ["ext:忽悠宇宙/skin/image/meng_furina/mengjvxing_achieve.jpg"]],
					["mengjvxing_fail", ["ext:忽悠宇宙/skin/image/meng_furina/mengjvxing_fail.jpg"]],
				],
				meng_leidianying: [
					['mengwumeng', ['ext:忽悠宇宙/skin/image/meng_leidianying/mengwumeng.jpg']],
					['meng_leidianying', ['die:meng_leidianying', 'img:meng_leidianying']],
				],
				meng_caomao: [
					["meng_caomao_shadow", ["ext:忽悠宇宙/skin/image/meng_caomao/meng_caomao_shadow.jpg", "die:meng_caomao"]],
					["meng_caomao_dead", ["ext:忽悠宇宙/skin/image/meng_caomao/meng_caomao_dead.jpg", "die:meng_caomao"]],
				],
				meng_bonisi: [
					["mengrongyan", ["ext:忽悠宇宙/skin/image/meng_bonisi/mengrongyan.jpg", "die:meng_bonisi"]],
					["meng_bonisi", ["img:meng_bonisi", "die:meng_bonisi"]],
				],
			},
		}
	})
	...
	translates.hyyzSort_ys = `<img src="${lib.assetURL}extension/忽悠宇宙/other/hyyzSort_ys.png" width="76" height="22">`
	translates.hyyzSort_ys_info = '原神·空月之歌'
	translates.hyyzSort_xt = `<img src="${lib.assetURL}extension/忽悠宇宙/other/hyyzSort_xt.png" width="76" height="22">`
	translates.hyyzSort_xt_info = '崩坏：星穹铁道'
	translates.hyyzSort_zzz = `<img src="${lib.assetURL}extension/忽悠宇宙/other/hyyzSort_zzz.png" width="76" height="22">`
	translates.hyyzSort_zzz_info = '绝区零'
	translates.hyyzSort_b3 = `<img src="${lib.assetURL}extension/忽悠宇宙/other/hyyzSort_b3.png" width="76" height="22">`
	translates.hyyzSort_b3_info = '崩坏三'
	translates.hyyzSort_other = `<img src="${lib.assetURL}extension/忽悠宇宙/other/hyyzSort_other.png" width="76" height="22">`
	translates.hyyzSort_other_info = '其他武将'
