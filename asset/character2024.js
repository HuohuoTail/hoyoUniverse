...
const characters = {
	2401: {
		hyyz_xt_ruanmei: ['阮·梅', ["female", "hyyz_xt", 3, ["hyyzpeiyu", "hyyznicha"], []], '紫灵谷的骊歌', '气质温婉优雅的学者，「天才俱乐部」#81号会员，生命科学领域的专家。<br>凭借天赋与惊人的执著得到了博识尊的瞩目，在秘密的角落开始了对生命本源的研究与探索。<br>并因此被黑塔邀请，同螺丝咕姆、斯蒂芬联合开发了「模拟宇宙」。<br>私下里，她十分喜爱传统戏剧与点心，对刺绣也很感兴趣。'],
		hyyzpeiyu: {
			audio: 2,
			enable: "phaseUse",
			usable: 1,
		...
		hyyz_xt_yinzhi: ['银枝', ["male", "hyyz_xt", 4, ["hyyzxinyang", "hyyzximei"], []], '紫灵谷的骊歌', '他为人正直、光明磊落，高贵的天性令人敬佩——一位游走宇宙间的独行者，坚定践行「纯美」。维护「纯美」在宇宙间的名誉，是银枝的职责：履行这一义务，起手需虔诚，落枪时则将要令人心悦诚服。'],
		hyyzxinyang: {
			audio: 4,
		...
		hyyz_b3_aiyi: ['爱衣', ["female", "hyyz_b3", 3, ["mengmiaobu", "mengyansuan"], []], '浮生亦', ''],
		mengmiaobu: {
			audio: 5,
			logAudio: () => [
		...
		hyyz_b3_re_xinyanzhilvzhe: ['薪炎之律者', ["female", "hyyz_b3", 4, ["mengliaohuang", "mengjingmang"], []], '拾壹', ''],//
		mengliaohuang: {
			audio: "mengweizhu",
			trigger: {
				global: "damageEnd",
			...
			async content(event, trigger, player) {
				...
				while (list.length > 0) {
					const cardx = list.shift();
					let cards = [ui.create.card(), ui.create.card()];
					cards[0].init(cardx);
					cards[1].init(cardx);
					if (trigger.source?.isAlive()) {
						const { bool } = await player
							.chooseBool('将' + get.translation(cardx) + '交给' + get.translation(trigger.source) + '，或点取消置入牌堆')
							.set('ai', () => get.attitude(player, trigger.source) > 0)
							.forResult()
						if (bool) {
							await trigger.source.gain(cards, 'gain2');
						} else {
							game.log(player, '将', cards, '随机插入牌堆');
							while (cards.length) {
								ui.cardPile.insertBefore(cards.shift().fix(), ui.cardPile.childNodes[get.rand(0, ui.cardPile.childNodes.length)]);
							}
		...
		hyyz_xt_sp_jingyuan: ['景元', ["male", "hyyz_xt", 5, ["menglaoshen", "mengguiqu"], ['zhu']], '微雨'],
		menglaoshen: {
			audio: 2,
			trigger: {
		...
		mengguiqu: {
			...
			mod: {
				targetInRange(card) {
					if (_status.event.skill == 'mengguiqv_backup') return true;
				},
			},
			ai: {
				fireAttack: true,
		...
		"menglaoshen_info": "劳神|锁定技，你于回合内[使用/弃置]牌后[失去/回复]1点体力。",
		"mengguiqu_info": "归去|你可以弃置两张类型相同的牌并摸一张牌，以将其中一张装备牌置入装备区，或视为使用或打出一张同类型即时牌。",


		hyyz_xt_sp_ruanmei: ['阮·梅', ["female", "hyyz_xt", 3, ["mengtansheng", "mengzidian"], []], '柚衣'],
		mengtansheng: {
			getLifeCard() {//获得一张随机点数，根据花色决定类型的生命牌
				...
				var name;
				switch (suit) {
					case 'heart': name = 'hyyz_taohuasu'; break;
					case 'club': name = 'hyyz_meihuagao'; break;
					case 'diamond': name = 'hyyz_caomeibing'; break;
					default: name = 'hyyz_chashaobao'; break;
				};
			...
			subSkill: {
				add: {
					...
					filter(event, player) {
						...
						if (!event.cards.some(card => ['hyyz_taohuasu', 'hyyz_meihuagao', 'hyyz_caomeibing', 'hyyz_chashaobao'].includes(card.name))) return false;
						return true;
					},
					direct: true,
					content() {
					...
					ai: {
						...
						skillTagFilter(player, tag, arg) {
							return ['hyyz_taohuasu', 'hyyz_meihuagao', 'hyyz_caomeibing', 'hyyz_chashaobao'].includes(arg.card.name);
						},
					},
				},
				usable: {},
		...
		hyyz_xt_guinaifen: ['桂乃芬', ["female", "hyyz_xt", 3, ["mengzhuyi", "menghenhuo", "mengtangcai"], []], '柚衣'],
		mengzhuyi: {
			audio: 4,
			trigger: {
		...
		hyyz_xt_luka: ['卢卡', ["male", "hyyz_xt", 4, ["menghanxin", "mengquanzhi"], []], '绯色愫'],
		menghanxin: {
			audio: 5,
			logAudio: () => [
		...
		hyyz_ys_baizhu: ['白术', ["male", "hyyz_ys", 3, ["mengzhenyao", "mengwenji"], []], '冷若寒'],
		mengzhenyao: {
			audio: 3,
			enable: "phaseUse",
			usable: 1,
			...
			async content(event, trigger, player) {
				...
				const { index: index1 } = await player.chooseControl().set('choiceList', ['将一张手牌替换为【毒】', '创造一张【无中生有】', '与对方交换手牌']).set('forced', false).set('ai', function () {
				...
				const { index: index2 } = await target.chooseControl().set('choiceList', ['将一张手牌替换为【毒】', '创造一张【无中生有】', '与对方交换手牌']).set('forced', false).set('ai', function () {
					var targetx = _status.event.targetx;
					var player = _status.event.player;
					var value = 0;
					var cards0 = player.getCards('h');
				...
				if (index1 == 0) {
					if (player.countCards('h')) {
						const { cards } = await player.chooseCard('h', '将一张牌替换为【毒】', true).forResult();
						if (cards) {
				...
				if (index2 == 0) {
					if (target.countCards('h')) {
						const { cards } = await target.chooseCard('h', '将一张牌替换为【毒】', true).forResult();
						if (cards) {
							cards[0].init([get.suit(cards[0]), get.number(cards[0]), 'du']);
						}
					}
		...
		hyyz_xt_sp_fuxuan: ['符玄', ["female", "hyyz_xt", 6, ["mengchitong", "mengxizhi"], []], '微雨'],
		mengchitong: {
			audio: 'mengjianzhi',
			trigger: {
		...
		hyyz_ys_sb_nahida: ['纳西妲', ["female", "hyyz_ys", 3, ["mengxushi", "mengnanke", "mengzhezhi"], []], '柚衣'],
		mengxushi: {
			audio: 'mengxvkong',
			trigger: {
		...
		hyyz_ys_furina: ['芙宁娜', ["female", "hyyz_ys", 3, ["mengjvxing", "mengshenyi"], ['zhu',]], '踽踽独行-尾巴酱', `
			特别鸣谢——<br>
			...
			subSkill: {
				achieve: {
					...
					async content(event, trigger, player) {
						...
						player.changeSkin({ characterName: "hyyz_ys_furina" }, "mengjvxing_achieve");
				...
				fail: {
					...
					async content(event, trigger, player) {
						...
						player.changeSkin({ characterName: "hyyz_ys_furina" }, "mengjvxing_fail");
						...
						for (let target of players) {
							...
							const { control } = await target
								...
								.set("prompt", "水淹七军").set("prompt2", "请选择一项：⒈弃置装备区里的所有牌；⒉受到1点雷电伤害。").forResult();
		...
		mengshenyi: {
			...
			subSkill: {
				...
				draw: {
					...
					async content(event, trigger, player) {
						const result = await player.draw(player.countEnabledSlot() - player.countCards('h')).forResult()
						if (result) {
							lib.translate['visible_hyyz_ys_furina'] = '明·芙';
							player.addShownCards(result, 'visible_hyyz_ys_furina');
						}
					},
				}
			},
		...
		hyyz_ys_zhongli: ['钟离', ["male", "hyyz_ys", 4, ["mengqiyue", "menglvheng", "mengminhui"], ['zhu',]], '鼎铸山河-尾巴酱', `
					特别鸣谢——<br>
					代码导师：<font color=#ff0dac>紫灵谷的骊歌</font><br>
					引路老师：<font color=#ff0dac>萨巴鲁酱</font><br>
					代码参考：<font color=#ff0dac>《忽悠宇宙》扩展</font><br>
		...
		hyyz_ys_nahida: ['纳西妲', ["female", "hyyz_ys", "3/4", ["mengkunchu", "mengxvkong", "mengzhuSun"], ['zhu',]], '破土新芽-尾巴酱', `
					特别鸣谢——<br>
					代码导师：<font color=#ff0dac>紫灵谷的骊歌</font><br>
					引路老师：<font color=#ff0dac>萨巴鲁酱</font><br>
		...
		hyyz_ys_leidianying: ['雷电影', ["female", "hyyz_ys", 4, ["mengwuwang", "mengwuxiang"], ['zhu',]], '断目销魂-尾巴酱', `
					特别鸣谢——<br>
					代码导师：<font color=#ff0dac>紫灵谷的骊歌</font><br>
					引路老师：<font color=#ff0dac>萨巴鲁酱</font><br>
					代码参考：<font color=#ff0dac> 《忽悠宇宙》扩展</font><br>
		...
		mengwuxiang: {
			...
			filter: (event, player) => {
				let list = player.getStorage('mengwuwang'), keys = Object.keys(list);
				if (!keys.length) return !player.getEquips('hyyz_mengxiangyixin').length;
				else {
			...
			content: () => {
				...
				if (!keys.length) {
					...
					var card = get.cardPile(function (card) {
						return card.name.includes('hyyz_mengxiangyixin')
					}, 'field');
					if (!card) {
						for (var i of game.filterPlayer()) {
							var card = i.getCards('he', (card) => card.name.search('hyyz_mengxiangyixin') != -1)[0];
							if (card) break;
						};
					}
		...
		hyyz_ys_wendy: ['温迪', ["male", "hyyz_ys", 4, ["mengjvlan", "menggongdan", "mengqinxin"], ['zhu',]], '尾巴酱', `
					特别鸣谢——<br>
					代码导师：<font color=#ff0dac>紫灵谷的骊歌</font><br>
					引路老师：<font color=#ff0dac>萨巴鲁酱</font><br>
					代码参考：<font color=#ff0dac>《忽悠宇宙》扩展</font><br>
	...
	2402: {
		hyyz_xt_zhenliyisheng: ['真理医生', ["male", "hyyz_xt", 4, ["hyyzbianbo", "hyyzguina"], []], '紫灵谷的骊歌', '直率而自我的博识学会学者，常以奇怪的石膏头雕遮蔽面容。自幼便展露出过人的才智，如今却以「庸人」自居。坚信智慧与创造力并不为天才独有，致力于向全宇宙传播知识，医治名为愚钝的顽疾。'],
		hyyzbianbo: {
			audio: 4,
		...
		hyyz_xt_sp_huohuo: ['藿藿', ["female", "hyyz_xt", 3, ["hyyzweiqie", "hyyzxvxing"], []], '大勇若怯-尾巴酱', `
		...
		hyyz_xt_danhengbailu: ['丹恒&白露', ["double", "hyyz_xt", 4, ["mengwugui", "menggushen", "mengjuefeng"], []], '日玖阳气冲三关'],//
		mengwugui: {
			init: (player) => {
			...
			async content(event, trigger, player) {
				...
				const { targets } = await player.chooseTarget('无归：令一名其他角色执行其他项或进行【闪电】判定', lib.filter.notMe)
		...
		hyyz_ɸ_pink: ['颦客', ["female", "shu", 7, ["mengyingzhu", "mengqiongpi"], []], '萨巴鲁酱', ''],
		mengyingzhu: {
			unique: true,
			audio: 6,
			logAudio: () => false,
				...
				die: {
					...
					mod: {
						cardSavable(card, player) {
							if (player.name != 'hyyz_ɸ_pink') return;
		...
		hyyz_xt_natasha: ['娜塔莎', ["female", "hyyz_xt", 3, ["mengyizhex", "mengjiuhu"], []], '梦海离殇'],
		mengjiuhu: {
			audio: 2,
			marktext: "护",
			intro: {
	...
	2403: {
		hyyz_b3_zhongyanzhilvzhe: ['终焉之律者', ["female", "hyyz_b3", 4, ["mengzhaoxi", "mengpingji", "mengcifan"], []], '拾壹'],
		mengzhaoxi: {
			audio: 3,
			mark: true,
			marktext: "☯",
		...
		hyyz_b3_xinyanzhilvzhe: ['薪炎之律者', ["female", "hyyz_b3", 4, ["mengweizhu", "mengbinye"], []], '拾壹'],
		mengweizhu: {
			audio: 4,
			trigger: {
				global: 'useCardAfter',
		...
		hyyz_xt_shiwaluo: ['史瓦罗', ["male", "hyyz_xt", 4, ["mengshouhu", "mengbushu"], []], '咩阿栗诶'],//
		mengshouhu: {
			audio: 'mengweijia',
			trigger: {
				global: 'damageBegin4'
		...
		hyyz_xt_wangxiayitong: ['王下一桶', ["none", "hyyz_xt", 4, ["mengmoli"], []], '咩阿栗诶'],//
		mengmoli: {
			enable: "phaseUse",
			usable: 1,
			viewAs: {
		...
		hyyz_xt_sp_heitiane: ['黑天鹅', ["female", "hyyz_xt", 3, ["mengshuijing", "mengliuguang", "mengzhenzhao"], []], '柚衣'],
		mengshuijing: {
			audio: 4,
			init: (player) => {
		...
		hyyz_xt_sb_jingliu: ['镜流', ["female", "hyyz_xt", 4, ["mengzuanyue", "mengshishui"], []], '微雨'],
		mengzuanyue: {
			audio: 2,
			enable: ["chooseToUse", "chooseToRespond"],
			filter(event, player) {
		...
		hyyz_xt_sangbo: ['桑博', ["male", "hyyz_xt", 4, ["mengdahun", "mengzishu"], []], '浮生亦', ''],
		mengdahun: {
			audio: 5,
			logAudio: () => [
		...
		hyyz_ys_leidianzhen: ['雷电真', ["female", "hyyz_ys", 4, ["mengjiaohui", "mengzhufu", "mengxvyu"], []], '梦海离殇'],//
		mengjiaohui: {
			enable: 'phaseUse',
			filter(event, player) {
		...
		hyyz_b3_aboniya: ['阿波尼亚', ["female", "hyyz_b3", 3, ["mengjielv", "mengzuiyuan", "mengganzhao"], []], '沧海依酥'],
		mengjielv: {
			audio: 2,
			trigger: {
				player: "damageEnd",
			...
			async content(event, trigger, player) {
				...
				if (trigger.source?.isIn()) {
					const { cards } = await player.chooseCard('将一张牌置于伤害来源武将牌旁作为“律”', 'he', true).forResult()
					if (cards) {
						trigger.source.addToExpansion(cards, player, 'giveAuto').gaintag.add('mengjielv2');
						trigger.source.addSkill('mengjielv2')
					}
				}
			},
			ai: {
			...
			mod: {
				cardEnabled(card, player) {
					if (game.hasPlayer((current) => current.hasSkill('mengjielv')) && player.getExpansions('mengjielv2').length) {
						if (player.getExpansions('mengjielv2').some(cardx => get.type2(cardx) == get.type2(card))) return false;
					}
				},
				cardSavable(card, player) {
					if (game.hasPlayer((current) => current.hasSkill('mengjielv')) && player.getExpansions('mengjielv2').length) {
		...
		mengzuiyuan: {
			...
			async content(event, trigger, player) {
				for (let current of event.targets) {
					const num = current.getExpansions('mengjielv2').length;
					const { cards } = await current
						.chooseCard(num, (card, player, target) => {
							return _status.event.player.getExpansions('mengjielv2').some(i => get.type2(i) == get.type2(card))
						})
						.set('prompt', '将' + get.cnNumber(num) + '张“律”包含类型的牌交给' + get.translation(player) + '，否则你失去1点体力且其获得所有“律”')
						.set('ai', () => -1)
						.forResult()
					if (cards) {
						...
						await player.gain(current.getExpansions('mengjielv2'), 'giveAuto');
						current.removeSkill('mengjielv2')
					}
				}
			},
			"_priority": 0,
		...
		mengganzhao: {
			...
			async content(event, trigger, player) {
				...
				const list = [];
				if (target.countCards('he') >= 2) list.push('交给其两张牌');
				list.push('受到1点伤害')
				list.push('翻面')
				const { control } = await target.chooseControl(list).forResult()
				switch (control) {
					case '交给其两张牌': await target.chooseToGive(player, 'he', 2, true); break;
					case '受到1点伤害': await target.damage(); break;
					case '翻面': await target.turnOver(); break;
				}
			},
			mod: {
				ignoredHandcard(card, player) {
		...
		"mengjielv_info": "戒律|当你受到1点伤害后，你可以摸一张牌并将一张牌置于伤害来源的武将牌旁，称为“律”。（有“律”的角色不能使用或打出与其拥有的“律”类型相同的牌）",
		mengjielv2_info: '律|',
		"mengzuiyuan_info": "罪渊|回合结束时，所有拥有“律”的角色需交给你X张与其拥有的“律”类型相同的牌，否则其失去1点体力。然后你获得其所有“律”。",
		"mengganzhao_info": "感召|出牌阶段限一次，你可以将一张手牌交给一名其他角色，然后令其选择一项：1.交给你两张牌；2.受到1点伤害；3.翻面。",

		hyyz_b3_ailixiya: ['爱莉希雅', ["female", "hyyz_b3", 3, ["mengwuxia", "mengailian", "mengzhenwo"], []], '沧海依酥'],
		mengwuxia: {
			trigger: {
				player: "damageBefore",
				source: "damageBefore",
		...
		mengzhenwo: {
			...
			forceDie: true,
			async cost(event, trigger, player) {
				event.result = await player.chooseTarget('令一名其他角色加1点体力上限并摸三张牌', true, lib.filter.notMe).set('ai', (target) => get.attitude2(target)).forResult()
			},
			async content(event, trigger, player) {
				await event.targets[0].gainMaxHp();
		...
		"mengwuxia_info": "无瑕|锁定技，你造成或受到的伤害均视为无来源伤害。",
		"mengailian_info": "爱恋|当一名角色受到伤害时，你令其摸一张牌。然后若此伤害为无来源伤害，你获得其区域内的一张牌；否则你获得伤害来源区域内的一张牌。",
		"mengzhenwo_info": "真我|锁定技，当你死亡时，你令一名其他角色加1点体力上限并摸三张牌。",

		hyyz_b3_geleixiu: ['格蕾修', ["female", "hyyz_b3", 3, ["mengfanxing"], []], '沧海依酥'],
		"mengfanxing": {
			forced: true,
			audio: 2,
			trigger: {
			...
			async content(event, trigger, player) {
				...
				if (trigger.source == player && !player.hasSkill('mengtiaohe')) {
					await player.addSkills('mengtiaohe')
				} else await player.draw();
			},
			"_priority": 0,
		},
		"menghuimeng": {
			...
			async cost(event, trigger, player) {
				...
				const { control } = await player.chooseControl(list, 'cancel2').set('prompt', '选择获得' + get.translation(trigger.source) + '的一个技能').set('forceDie', true).set('ai', function () {
			...
			async content(event, trigger, player) {
				...
				game.broadcastAll(function (skill) {
					...
					for (var i of list) {
						...
						info.audioname2.hyyz_b3_geleixiu = 'menghuimeng';
					}
				}, event.cost_data);
			},
			mark: true,
		...
		"mengtiaohe": {
			...
			async cost(event, trigger, player) {
				...
				const { control } = await player
					.chooseControl(list).set('choiceList', [
						'令该角色失去一点体力',
						'防止此伤害并令其弃置两张牌',
						'背水！你翻面并执行所有选项',
					]).forResult();
			...
			async content(event, trigger, player) {
				...
				if (control == '背水！') player.turnOver();
				if ((control == '选项一' || control == '背水！')) await trigger.player.loseHp();
				if ((control == '选项二' || control == '背水！') && trigger.player.countDiscardableCards(player, 'he') > 0) {
					trigger.cancel();
		...
		"mengfanxing_info": "繁星|锁定技，当你受到/造成伤害时，获得技能“绘梦”/“调和”，若已拥有则摸一张牌",
		"menghuimeng_info": "绘梦|限定技，当你受到伤害时，你可以获得伤害来源的一个技能（不包括主公技、觉醒技和限定技）。",
		"mengtiaohe_info": "调和|每回合限一次，当你造成伤害时你可选择一项：①令该角色失去一点体力；②防止此伤害并令其弃置两张牌；③背水：你翻面并执行所有选项。",

		hyyz_b3_wu_hua: ['华', ["female", "hyyz_b3", 4, ["mengfusheng", "mengguiyi", "mengduao"], []], '沧海依酥'],
		"mengfusheng": {
			mark: true,
			locked: true,
			zhuanhuanji: true,
		...
		"mengfusheng_info": "浮生|转换技，锁定技，阴：锁定技，出牌阶段开始时，你摸一张牌，然后本回合使用牌无次数限制，且不可被相应。阳：锁定技，出牌阶段开始时，你弃置一张牌，然后本回合使用的牌无距离限制，且造成的伤害+1。",
		"mengguiyi_info": "歸憶|①当你使用不同牌名的牌后，你记录此牌名。②结束阶段，你可在〖追忆〗的记录中减少最多1张种牌的牌名，并从牌堆中获得所有本回合打出牌名相同的牌",
		"mengduao_info": "渡鏖|限定技，当你处于濒死状态时，你可以增加一点体力上限，并将体力回复至体力上限，然后将手牌补至体力上限。",
		...
		"mengyuxiang_info": "余响|当你使用或打出牌时，若你的手牌数小于你的体力值，你可以将手牌摸至体力上限。",
		"mengcanmeng_info": "残梦|每回合限一次。当你受到伤害时，若你的手牌数不小于你的体力值，你可以视为对伤害来源使用一张【杀】。",
		"mengjiushi_info": "救世|主公技，锁定技。游戏开始时，你加X点体力上限（X为场上其他崩势力的角色数）；当一名崩势力角色死亡时，你减1点体力上限。",

		hyyz_b3_kesimo: ['科斯魔', ["male", "hyyz_b3", 4, ["mengxuguang", "mengtieyan"], []], '沧海依酥'],
		"mengxuguang": {
			forced: true,
			audio: 2,
		...
		"mengxuguang_info": "旭光|锁定技。每当你造成伤害时，受到伤害的角色获得一个“裂”标记，其回合开始时移除所有“裂”标记，然后失去一点体力。",
		"mengtieyan_info": "餮宴|当一名角色移除“裂”标记时，你可以摸等同于移除的“裂”标记数的牌，并且你获得其一个技能（不包括主公技、觉醒技和限定技），直到你的下回合结束，若其因移除“裂”标记而进入濒死状态则改为获得技能直到游戏结束。",

		hyyz_b3_meibiwusi: ['梅比乌斯', ["female", "hyyz_b3", 4, ["mengqiying", "mengwuxian"], []], '沧海依酥'],
		"mengqiying_mark": {
			marktext: "噬",
			intro: {
				name: "噬",
		...
		"mengqiying_info": "栖影|出牌阶段限一次，你可以弃置一张普通锦囊，然后令一名其他角色获得“噬”标记:拥有“噬”记的角色准备阶段选择一项:1、跳过判定阶段和出牌阶段:2、跳过摸牌阶段和弃牌阶段。 回合结束时移除“噬”标记，若其本回合没有使用或打出过手牌则失去一点体力，若其没有获得过牌或弃置过手牌则你弃置其两张牌。",
		"mengwuxian_info": "无限|锁定技。当你进入濒死状态时，你减1点体力上限，然后将体力回复至体力上限，并将手牌补至体力上限。",

		hyyz_b3_paduo: ['帕朵', ["female", "hyyz_b3", 3, ["mengkongmeng", "menglveji", "menghuoyin"], []], '沧海依酥'],
		mengkongmeng: {
			forced: true,
			trigger: {
		...
		"mengkongmeng_info": "空梦|锁定技：①当你武将牌背面朝上时，你不能成为其他角色使用牌的目标；②摸牌阶段，你额外摸两张牌；③你的手牌上限+X（X为你的体力上限）。",
		"menglveji_info": "掠集|出牌阶段限一次，你可以获得至多两名其他角色区域内的各一张牌，然后这些角色可以依次对你使用一张【杀】。",
		"menghuoyin_info": "祸引|当你成为其他角色使用牌的唯一目标时，你可以将一张手牌交给一名其他角色，令其代替你成为此牌的目标，然后你翻面。",

		hyyz_b3_qianjie: ['千劫', ["male", "hyyz_b3", 4, ["mengfenshen", "mengbengluo"], []], '沧海依酥'],
		"mengfenshen": {
			audio: 2,
			trigger: {
				player: "phaseUseBegin",
		...
		"mengfenshen_info": "焚身|出牌阶段开始时，你可以对自己造成一点火焰伤害，然后本回合每当你造成伤害时，你摸一张牌。",
		"mengbengluo_info": "崩落|觉醒技。当你体力值降至2或者2以下时，你增加一点体力上限并回复满体力，将手牌摸至体力上限，然后你获得技能“鏖灭”。",
		"mengaomie_info": "鏖灭|锁定技。回合内每当你使用杀或伤害类锦囊时，若你体力值不为1则你失去一点体力，然后此牌不可被响应且造成的伤害加1",

		hyyz_b3_su: ['苏', ["male", "hyyz_b3", 4, ["mengmiyan", "mengtianhui", "mengyizhe"], []], '沧海依酥'],
		mengmiyan: {
			audio: 1,
			trigger: {
				player: "phaseZhunbeiBegin",
			...
			async cost(event, trigger, player) {
				event.result = await player
					.chooseTarget(get.prompt('mengmiyan'))
					.set('ai', (target) => -get.attitude2(target))
					.forResult()
			},
			async content(event, trigger, player) {
		...
		"mengmiyan_info": "密言|准备阶段，你可以选择一名角色。直到你下回合开始，当其造成1点伤害后，其须选择一项：1.令受伤角色摸一张牌，然后你摸一张牌；2.弃置一张牌，然后你获得其弃置的牌。",
		...
		hyyz_b3_sp_weierwei: ['维尔薇', ["female", "hyyz_b3", 3, ["mengyuxi", "mengluoxuan", "mengwuzhuang"], []], '沧海依酥'],
		mengyuxi: {
			forced: true,
			mod: {
		...
		"mengyuxi_info": "愚戏|锁定技。你的【杀】均视为【闪】；其他角色计算与你的距离+X（X为你装备区里的牌数）。",
		"mengluoxuan_info": "螺旋|①出牌阶段，你可以弃置一张装备牌，视为使用一张无距离限制且不计入次数的【杀】。②当你于回合外受到伤害时，你可以弃置一张装备牌，然后摸等同于伤害值的牌并防止此伤害。",
		"mengwuzhuang_info": "武装|出牌阶段开始时，你可以弃置一张手牌，然后从牌堆或弃牌堆中随机获得一张装备牌。",

		hyyz_b3_shiyuanzhilvzhe: ['始源之律者', ["female", "shen", 3, ["mengzhuguang", "mengnisu", "mengyingwu"], []], '沧海依酥'],
		mengzhuguang: {
			audio: 2,
			forced: true,
		...
		"mengzhuguang_info": "逐光|锁定技，当武器牌进入你的装备区时你弃置之；每当你造成或者受到伤害时，你获得一个“瑕”标记，你至多拥有13个“瑕”标记。",
		"mengnisu_info": "逆溯|准备阶段，你可以选择任意名其他角色，并移除等量的“瑕”标记，然后本回合你对这些角色使用牌无距离限制，且其不可响应；每个角色回合各限一次，你可以移除一个“瑕”标记。①、视为使用一张基本牌；②、视为使用一张普通锦囊牌。",
		"mengyingwu_info": "影舞|结束回合开始时，若你拥有的“瑕”标记数不小于当前存活人数，则你可以弃置任意数量的“瑕”标记，并视为对等量的其他角色，以此使用一张无距离限制的雷杀，此杀每造成一点伤害，你摸一张牌。",

		hyyz_b3_yidian: ['伊甸', ["female", "hyyz_b3", 3, ["mengyuepu", "menghuangjin"], []], '沧海依酥'],
		"menghuangjin": {
			audio: 2,
			forced: true,
			intro: {
		...
		"menghuangjin_info": "黄金|锁定技。每当你或拥有乐谱给予技能的角色（该角色不能是你）造成一点伤害时你获得一个“音”标记；每当你移除“音”标记时你摸等同于移除的“音”标记数的牌（至多摸五张）。",
		"mengyuepu_info": "乐谱|回合结束时，你移除所有的“音”标记，若移除的标记数为单数，你令一名角色获得技能“和弦”，直到你下个出牌阶段结束；若为双数则令一名角色获得技能“变奏”直到你的下个回合出牌阶段结束。",
		"menghexian_info": "和弦|锁定技。当你受到伤害时，令此伤害-1，然后你摸一张牌。",
		"mengbianzou_info": "变奏|锁定技。当你造成伤害时，若此伤害不大于1，则此伤害+1，然后你摸一张牌",

		hyyz_b3_ying: ['樱', ["female", "hyyz_b3", 4, ["mengsenluo", "mengshana"], []], '沧海依酥'],
		"mengsenluo": {
			audio: 2,
			trigger: {
		...
		"mengsenluo_info": "森罗|锁定技。当你使用的牌被响应或你响应其他角色使用的牌时你摸一张牌。",
		"mengshana_info": "刹那|限定技。出牌阶段结束时，你可以跳过弃牌阶段，然后进行一个额外的摸牌阶段和出牌阶段，此出牌阶段其他角色非锁定技失效，你使用的牌没有次数和距离限制，且不能被响应，然后回合结束时你进入濒死状态。",
	...
	2404: {
		hyyz_xt_huahuo: ['花火', ["female", "hyyz_xt", 3, ["hyyzjiaoshi", "hyyzkehun", "hyyzjiamian"], []], '紫灵谷的骊歌', '「假面愚者」的成员之一，难以捉摸，不择手段。<br>危险的戏剧大师，沉迷于扮演，身怀千张假面，能化万种面相。<br>财富、地位、权利…于花火而言都不重要，能让她出手的，唯有「乐趣」。'],
		hyyzjiaoshi: {
			audio: 4,
			enable: ["chooseToUse", "chooseToRespond"],
			usable: 1,
		...
		hyyz_b3_sp_leidianyayi: ['雷电芽衣', ["female", "hyyz_b3", 3, ["mengwanzui", "mengchangci", "mengguxing"], []], '微雨', ''],
		mengwanzui: {
			audio: 'menglizui',
			trigger: {
		...
		hyyz_xt_lingke: ['铃可', ['female', 'hyyz_xt', '1/3/2', ["mengjuejing", "mengxueyuan", "mengqiusheng"], []], '心之所向_星之所向'],
		mengjuejing: {
			audio: 2,
			trigger: {
				player: 'loseHpBefore'
		...
		mengyanglu: {
			...
			async content(event, trigger, player) {
				...
				const { links: gain } = await player.chooseButton(true, ['获得一张牌', cards])
					.forResult();
				if (gain) {
					cards.remove(gain[0]);
	...
	2405: {
		hyyz_xt_huangquan: ['黄泉', ["female", "hyyz_xt", 1, ["hyyzlunshi", "hyyzxvwu", "hyyzanshang"], []], '紫灵谷的骊歌', '自称「巡海游侠」的旅人，本名不详。身佩一柄长刀，独行银河。<br>淡漠寡言，剑出如紫电般迅猛，却从来只以刀鞘战斗，收而不发。'],
		hyyzlunshi: {
			audio: 2,
			trigger: {
				global: "gainAfter",
			...
			async content(event, trigger, player) {
				...
				if (trigger.player == player) {
					...
					if (!player.storage.hyyzxvwu.length) {
						...
						switch (num) {
							case 1: {
								lib.translate['hyyz_xt_huangquan'] = '■■';
							...
							case 4: {
								player.node['avatar'].setBackgroundImage('extension/忽悠宇宙/asset/hyyz/image/hyyz_xt_huangquan_0.jpg');
								game.log(player, "#r的原画信息消散了");
								break;
							}
							case 5: {
								lib.characterTitle.hyyz_xt_huangquan = '#r■■■■■■，<br>■■■■，■■■■……<br>■■■■，■■■■。'
		...
		hyyz_xt_shajin: ['砂金', ['male', 'hyyz_xt', 4, ['hyyzniming', 'hyyzpoai', 'hyyzqingzhi'], ['zhu',]], '紫灵谷的骊歌', '星际和平公司「战略投资部」的高级干部，「石心十人」之一，基石为「诡弈砂金」。<br>个性张扬的风险爱好者，时常面带笑容，真心却难以揣测。<br>靠着同命运的博弈赢得如今的地位，将人生视作一场高风险、高回报的投资，而他向来游刃有余。'],
		hyyzniming: {
			audio: 2,
			init(player) {
		...
		hyyzpoai: {
			...
			subSkill: {
				audio: {
					...
					async content(event, trigger, player) {
						const target = event.targets[0];
						const { links } = await player.choosePlayerCard('破霭：选择一张牌用于拼点', target, 'ej', true).forResult();
						if (links) {
	...
	2406: {
		hyyz_xt_liuying: ['流萤', ["female", "hyyz_xt", "4/5", ["hyyzranshang", "hyyzyingzhu"], []], '紫灵谷的骊歌', '星核猎手成员，身着机械装甲「萨姆」的少女。<br>以兵器的身份诞生，因基因改造罹患「失熵」的痛苦。<br>为寻求生命的意义加入星核猎手，不断追逐违抗命运的方式。'],
		hyyzranshang: {
			audio: 8,
			forced: true,
		...
		hyyzranshang_info: "燃熵|锁定技，你每回合结束时【崩坏】。你的体力值/装备区的牌数始终为另一项的空缺数。",
	...
	2407: {
		...
		hyyz_xt_sp_liuying: ['流萤', ["female", "hyyz_xt", 3, ["mengxinying", "mengliuying"], []], '七夕月', ''],
		"mengxinying": {
			audio: 'mengliuguangzhuhuo',
			init(player) {
	...
	2408: {
		hyyz_ys_sp_furina: ['芙宁娜', ['female', 'hyyz_ys', 3, ['hyyzshenyi', 'hyyzmantian'], ['zhu',]], '踽踽独行-尾巴酱', '芙芙秉苍生之任，安危系于一身。承百年遗志，勉为水神，丝毫无差，惧惊天理。其能策勉，苦心孤诣，然众有不明者，欲揭其伪。天怒而降罚，芙芙由神降人。百年经营，心系苍生未改。芙芙之伟，在于牺牲，愿世人皆能明其志，共筑太平。'],
		hyyzshenyi: {
			audio: 8,
			logAudio: () => false,
			trigger: {
				...
				if (shown.length == 1) {
					...
					if (player.countCards('h', (card) => shown[0] != get.is.shownCard(card)) > 0) {
						const cards = player.countCards('h', (card) => shown[0] != get.is.shownCard(card)) == 1
							...
							:
							(await player.chooseCard('h', true, '神仪：重铸一张异置牌', (card) => shown[0] != get.is.shownCard(card)).forResult()).cards;
						if (cards) {
		...
		hyyzmantian: {
			...
			filter(event, player) {
				...
				if (num < storage) {//减少
					player.changeSkin({ characterName: "hyyz_ys_sp_furina" }, "hyyzshenyiremove");
					return true;
				} else if (num == storage) {//不变
					if (lib.config['extension_忽悠动态包_enable'] && ['identity', 'versus', 'boss', 'doudizhu', 'single', 'brawl'].includes(get.mode())) {
						player.node['avatar' + (get.nameList(player).indexOf('hyyz_ys_sp_furina') ? 2 : '')].setBackgroundImage('extension/忽悠动态包/image/hyyz_ys_sp_furina.gif');
					} else {
						player.changeSkin({ characterName: "hyyz_ys_sp_furina" }, "hyyz_ys_sp_furina");
					}
				} else if (num > storage) {//增加
					player.changeSkin({ characterName: "hyyz_ys_sp_furina" }, "hyyzshenyiadd");
		...
		hyyz_ys_sp_zhongli: ['钟离', ['male', 'hyyz_ys', 4, ['hyyzfuqiang', 'hyyzwanglong'], ['zhu',]], '鼎铸山河-尾巴酱', '钟离者，神而帝者也。御国以明，擅通财器，璃月因之昌盛。故，税入既足，内可安民之生，外可驱邪避患。若成主公，每轮多决机宜，日理万机，尽瘁至形神磨损。<br>钟离者，帝而神者也。神荫广被，民众无忧，渐失独立之能。然，世事变迁，上有七星夺权，下有民众自立。纵国重生，方能化龙而翔，不受羁绊，翱翔于九天之上。'],
		hyyzfuqiang: {
			audio: 9,
			global: 'hyyzfuqiang2'
		}, hyyzfuqiang2: {
		...
		hyyz_ys_sp_nahida: ['纳西妲', ['female', 'hyyz_ys', '3/5', ['hyyzsanling', 'hyyzpogu'], ['zhu',]], '破土新芽-尾巴酱', '本、自、我三精，各表人性之异。<br>本乃快乐之渊，唯兴是图，徒余兽性。<br>自然现实之丈，拮抗两我，律衡一体。<br>超则至善之晷，良心为规，理念为导。<br>小草居深宫，以虚空之力，察尘世众心。塑得超我非凡，遗失本我之念，而况自我，失衡则危。<br>或借他人援手，或以自身省之，寻得本性，觉醒真我。此乃纳西妲成长之路，自我救赎之旅，待三我和谐，方造新神。'],
		hyyzsanling: {
			forced: true,
		...
		hyyzpogu: {
			...
			async cost(event, trigger, player) {
				...
				if (control == '废除') {
					...
					list.sort();
					equip = (await player.chooseControl(list, 'cancel2').set('prompt', '废除一个装备栏').forResult()).control;
					event.result = {
						bool: equip != 'cancel2',
					...
					list.sort();
					equip = (await player.chooseControl(list, 'cancel2').set('prompt', '恢复一个装备栏').forResult()).control;
					event.result = {
						bool: equip != 'cancel2',
		...
		hyyz_ys_sp_leidianying: ['雷电影', ['female', 'hyyz_ys', 4, ['hyyzpaoying', 'hyyzhuanshi'], ['zhu',]], '断目销魂-尾巴酱', '稻妻之地，本归一瞬；影武凄厉，困于永恒。意图定世之乱象，止时之流转。然失之常伴，如昼夜更迭，春秋代序，不可抗拒。影明此理，欲以神力锁永恒于瞬息。殊不知，失之于外，得之于内，万物非止，唯心永恒。以失为鉴，以梦当先，若世间无失，何知得之可贵？珍惜当下，亦可传承不息。'],
		hyyzpaoying: {
			audio: 1,
			trigger: {
				global: "phaseBefore",
		...
		hyyz_xt_sb_ruanmei: ['阮·梅', ['female', 'hyyz_xt', 3, ['mengshuanghuaraobaiwei', 'mengdanqinghuiqianqiu', 'mengrouyifuwansheng'], []], '日玖阳气冲三关', ''],
		...
		mengdanqinghuiqianqiu: {
			...
			async content(event, trigger, player) {
				...
				if (control == '分配体力') {
					...
					if (cards) {
						const { targets: targets2 } = await player
							.chooseTarget('分配' + get.translation(target) + '的' + get.translation(cards) + '给：')
							.set('filterTarget', (card, player, target) => target != event.targets[0])
							.set('ai', target => get.attitude2(target))
							.forResult();
						if (targets2) {
							const target2 = targets2[0];
							target.line(target2);
							await target2.gain(cards, target, 'give');
						}
					}
				};
		...
		menghuahuoyouyi: {
			...
			async content(event, trigger, player) {
				const { bool } = await trigger.target.chooseBool('是否放弃响应？').set('ai', () => false).forResult();
				if (bool) {
		...
		"menghuahuoyouyi_append": "你使用被点燃的牌指定唯一目标后，目标可以放弃响应，路径上的角色可点燃手牌。",

		hyyz_xt_sp_botiou: ['波提欧', ['male', 'hyyz_xt', 4, ['mengxunhai', 'menghuxing'], []], '千秋万叶', ''],
		mengxunhai: {
			audio: 4,
			forced: true,
			trigger: {
		...
		"menghuxing_info": "呼星|准备阶段，若你没有“绝命”，你可以摸两张牌，然后将两张牌当【决斗】使用，并令【决斗】的双方获得技能“绝命”直至其中一方死亡。若如此做，你本回合判定阶段改为摸牌阶段。",
		"mengjueming_info": "绝命|锁定技，你使用牌只能指定其他拥有“绝命”的角色为目标，且对其使用牌无距离和次数限制。",

		hyyz_b3_leidianyayi: ['雷电芽衣', ['female', 'hyyz_b3', 5, ['menglizui', 'mengchengshi'], []], '灯姐'],
		menglizui: {
			marktext: '罪',
			intro: {
				name: '逦罪',
		...
		hyyz_ys_sb_zhongli: ['钟离', ['male', 'hyyz_ys', 4, ['mengyouwen', 'mengbolun'], ['zhu']], '樾舟', ''],
		mengyouwen: {
			audio: 'menglvheng',
			init(player, skill) {
				player.storage[skill] = {};
		...
		hyyz_ɸ_quancong: ['全琮', ['male', 'wu', 4, ['mengbunao', 'mengnaohao'], []], '七夕月', '取上则可得中，取下则可得中，取中则可得中。'],
		mengbunao: {
			audio: ['xinyaoming'],
			enable: 'phaseUse',
			usable: 1,
	...
	2409: {
		...
		hyyzminHp_info: '体力下限|',
		hyyz_ɸ_mansui: ['满穗', ['female', 'hyyz_ɸ', 3, ['hyyzepiao', 'hyyzcimang'], []], '烟花易冷-尾巴酱', ''],
		hyyzepiao: {
			init(player) {
				player.minHp = 1;
			},
		...
		menglanzhanmo: {
			...
			async content(event, trigger, player) {//一名角色某色拼点牌在所有角色同色和异色均居多
				...
				while (count <= 3 && game.hasPlayer(current => player.canCompare(current))) {
					...
					const { player: card1, target: card2 } = await player.chooseToCompare(target).forResult();
					//给每名角色的黑++红++
					if (card1) {
				...
				after: {
					...
					async content(event, trigger, player) {
						...
						if (get.color(trigger.card1) == 'black' && get.color(trigger.card2) == 'black') {
							...
							const control = cards.length == 1 ? '获得' + get.translation(cards) : (await player.chooseControl('获得' + get.translation(cards), '此技发动次数+1').forResult()).control
							if (control == '此技发动次数+1') {
								player.storage.mengzhangloudie++;
								player.syncStorage('mengzhangloudie');
							} else {
		...
		hyyz_ɸ_chunjin_aiyafala: ['艾雅法拉', ['female', 'hyyz_ɸ', 3, ['menghuoshan', 'mengyunshuang', 'mengwushen'], []], '昨夜流萤', ''],
		menghuoshan: {
			nobracket: true,
			dutySkill: true,
			locked: false,
		...
		hyyz_xt_sb_fuxuan: ['符玄', ['female', 'hyyz_xt', 3, ['mengxingzhou', 'mengdengkan'], []], '三秋'],
		mengxingzhou: {
			nobracket: true,
			audio: 'mengqiongguan',
			trigger: {
			...
			async content(event, trigger, player) {
				...
				const { moved } = await player.chooseToGuanxing(3).forResult();
				if (moved?.[1].length > 0) {
		...
		mengdengkan: {
			...
			async precontent(event, trigger, player) {
				...
				delete event.result.skill;
				const { cards } = await player.chooseToDiscard('选择摸两张牌或弃置两张牌', 2, 'he').forResult()
		...
		hyyz_ys_fukaluosi: ['芙卡洛斯', ['female', 'hyyz_ys', '3/6', ['mengxuanzhong1', 'mengxuanzhong2'], ['zhu']], '冷若寒', ''],
		mengxuanzhong1: {
	...
	2410: {
		hyyz_xt_luanpo: ['乱破', ['female', 'hyyz_xt', 4, ['hyyzmeiran', 'hyyzzhunian'], []], '紫灵谷的骊歌'],//
		hyyzmeiran: { audio: 2, global: 'hyyzmeiran_buff' }, hyyzmeiran_buff: {
			...
			async content(event, trigger, player) {
				if (event.triggername == 'gainBefore') {
					...
					const 乱破 = game.findPlayer(current => current.hasSkill('hyyzmeiran'));
					if (乱破) 乱破.logSkill('hyyzmeiran', player);
					...
					if (trigger.hyyzmeiran_name) {
						...
						if (list) {
							const { links } = await 乱破
								...
								.set('att', get.attitude(乱破, player))
								.forResult();
							if (links) {
					...
					const { result: { bool, links } } = await 乱破
		...
		hyyz_xt_zhigengniao: ['知更鸟', ["female", "hyyz_xt", 3, ['hyyzgongming', 'hyyzlingzhong'], []], '紫灵谷的骊歌'],
		hyyzgongming: {
			audio: 5,
			...
			async content(event, trigger, player) {
				...
				switch (trigger.name) {
					...
					case 'phaseDiscard': {
						for (const current of currents) {
							if (!event.hyyzgongming) break;
							if (!current.countCards('h') || (!await current.chooseToDiscard('共鸣：弃置一张手牌', true).forResult()).cards) {
								await lib.skill.hyyzgongming.toOther(event, trigger, current);
							}
						}
						break;
...
`],
		hyyzwanmeng: {
			...
			async content(event, trigger, player) {
				...
				switch (num) {
					...
					case 2: {
						const { cards } = await player.chooseCard('挽梦：重铸二张牌', 2, true, 'he').forResult();
			...
			async content(event, trigger, player) {
				if (trigger.name == 'changeSkills') {
					const { index } = await player
						.chooseControlList(`归真：即将获得<span class='yellowtext'>${get.translation(trigger.addSkill)}</span>，请抉择`, ['失去1点体力', '改为获得〖制衡〗'], true, () => {
							const player = _status.event.player;
					...
					async content(event, trigger, player) {
						...
						if (targets) {
							...
							if (cards) {
								...
								const { bool } = await player.chooseBool('是否加1点手牌下限？').forResult();
								if (bool) {
		...
		mengduhua_append: '出牌阶段，你可以失去1点体力或减1点体力上限，令一名其他角色回复1点体力或加1点体力上限，该角色选择一项：①交给你一张【杀】且本回合内无距离与次数限制。②令你增加1点手牌下限。',
	},
	2411: {
		hyyz_xt_moze: ['貊泽', ['male', 'hyyz_xt', 4, ['mengcangfeng', 'mengniying', 'mengjiandai'], []], '冷若寒'],
		mengcangfeng: {
			audio: 2,
			init(player) {
		...
		hyyz_ys_kaqina: ['卡齐娜', ['female', 'hyyz_ys', 3, ['mengduoji'], []], '冷若寒'],
		mengduoji: {
			audio: 4,
			trigger: {
				player: ["phaseZhunbeiEnd", "phaseJudgeEnd", "phaseDrawEnd", "phaseUseEnd", "phaseDiscardEnd", "phaseJieshuEnd"],
		...
		hyyz_b3_sp_meibiwusi: ['梅比乌斯', ['female', 'hyyz_b3', 4, ['mengliutu', 'mengfeiyun', 'mengtui4yan'], []], '微雨'],
		mengliutu: {
			audio: 'mengqiying',
			trigger: {
				global: [
		...
		hyyz_b3_weierwei: ['维尔薇', ['female', 'hyyz_b3', '2/3/3', ['mengwanwo', 'mengwusheng'], []], '冷若寒', '我是谁？是涅槃的火凤，薄幸的佳俏，赴北的义士，逐日的神人，救世的良医，和合的贤者，残虐的恶徒，陷阵的英杰…还是，我自己？？'],
		mengwanwo: {
			audio: 'mengwuzhuang',
			trigger: {
				player: 'useCardAfter'
	...
	2412: {
		hyyz_ys_xigewen: ['希格雯', ['female', 'hyyz_ys', 3, ['mengyiyv', 'mengyvzan', 'mengyirong'], []], '奕水之安', ''],
		mengyiyv: {
			audio: 6,
			enable: 'chooseToUse',
			filter(event, player) {
		...
		hyyz_ɸ_dinyi: ['丁仪', ['male', 'hyyz_ɸ', 3, ['mengwendao'], []], '朝闻道，夕死可矣-尾巴酱', '朝闻道，夕死可矣'],
		mengwendao: {
			audio: 8,
			logAudio: () => [
			...
			async content(event, trigger, player) {
				...
				if (cards) {
					...
					while (list.some(card => player.hasUseTarget(card, false, false))) {
						let { links: use_card } = list.length == 1 ? [list[0]] : (await player
						...
						if (use_card?.length) {
							...
							const { bool } = await player.chooseUseTarget(use_card[0], true, false, 'nodistance').forResult();
							if (bool) num--;
						} else {
							break;
						}
		...
		hyyz_xt_wangguiren: ['忘归人', ['female', 'hyyz_xt', 3, ['mengruoxi', 'mengwuguiWGR'], []], '微雨'],
		mengruoxi: {
			audio: 4,
			trigger: {
		...
		hyyz_xt_sb_shajin: ['砂金', ['male', 'hyyz_xt', 4, ['mengcifu', 'mengsandu', 'mengyazhu'], []], '囚石铸金-鸦懿鸢霏'],
		mengcifu: {
			audio: 2,
			init(player) {
		...
		mengsandu: {
			...
			async cost(event, trigger, player) {
				...
				dialog.add(list);
				const { links: cards } = await player.chooseButton(dialog)
					.set('ai', (button) => {
		...
		mengyazhu: {
			...
			check(event, player) {
				...
				if (player.hasSkill('mengcifu')) return false;
			...
			ai: {
				...
				result: {
					player(player) {
						...
						if (player.hasSkill('mengcifu')) return -2.5
		...
		"mengcifu_info": "赐福|锁定技，你的判定牌花色始终为♠。当你体力值为1时，你失去此技能并将体力上限调整至游戏开始时。",
		...
		hyyz_ɸ_cuicui: ['翠翠', ['female', 'hyyz_ɸ', 3, ['hyyzgungunlai'], []], '生死远行-拾壹', '若还想与我相见，就来到我的梦里边。'],
		hyyzgungunlai: {
			nobracket: true,
			...
			subSkill: {
				...
				hp: {
					...
					async spinningHp(player, oldMaxHp, newMaxHp) {
						...
						const hpCard = function (newMaxHp, player) {
							...
							const fileName = `extension/忽悠宇宙/other/image/hyyz_ɸ_cuicui/${lib.config['extension_十周年UI_enable'] ? 'ten_' : ''}hp${newMaxHp}.jpg`;
						...
						setTimeout(() => {
							...
							dialog.open();
							//设置背景图
							card.style["background-image"] = `url(${lib.assetURL}extension/忽悠宇宙/other/image/hyyz_ɸ_cuicui/${lib.config['extension_十周年UI_enable'] ? 'ten_' : ''}hp${oldMaxHp}.jpg)`
					...
					async content(event, trigger, player) {
						...
						for (let i = 0; i < player.maxHp; i++) {
							image1 += `<img style = 'width:18px; vertical-align: middle;' src='${lib.assetURL}extension/忽悠宇宙/other/image/hyyz_ɸ_cuicui/hp.png'>`;
						}
						for (let i = 0; i < toMaxHp; i++) {
							image2 += `<img style = 'width:18px; vertical-align: middle;' src='${lib.assetURL}extension/忽悠宇宙/other/image/hyyz_ɸ_cuicui/hp.png'>`;
						}
				...
				friend: {
					...
					async content(event, trigger, player) {
						...
						player.showIdentity();
						await player.chooseUseTarget({ name: 'hyyz_jiwang', isCard: true }, true);
					}
				},
			}
		},
		...
		hyyz_zzz_xingjianya: ['星见雅', ['female', 'hyyz_zzz', 3, ['mengliyi', 'mengqixue', 'mengchenling'], ["clan:狐族"]], '九尾归庭-冷若寒', '「小雅知道英雄要做些什么吗？」<br>「要消灭所有坏人！」<br>「英雄可不是为了消灭什么诞生的，而是为了守护哦。」'],
		mengliyi: {
			audio: 4,
			backWater: true,
			mark: true,
		...
		mengchenling: {
			...
			audio: 4,
			audioname2: {
				hyyz_zzz_xingjianya: 'mengchenling_hyyz_zzz_xingjianya',
				hyyz_xt_yvkong: 'mengchenling_hyyz_xt_yvkong',
			},
			subSkill: {
				hyyz_zzz_xingjianya: { audio: 4 },
				hyyz_xt_yvkong: { audio: 4 },
			},
			clanSkill: true,
			mark: true,
			marktext: "☯",
			...
			intro: {
				...
				content(storage, player, skill) {
					...
					let other = '';
					if (get.nameList(player).includes('hyyz_zzz_xingjianya')) other += (lib.skill.mengchenling.backWater ? '背水：你移动此项至上个发动的技能。' : '');
					return main + other
				},
			},
			trigger: {
		...
		hyyz_xt_sp_daheita: ['大黑塔', ['female', 'hyyz_xt', 4, ['mengqiuda'], []], '三顾博识-鸦懿鸢霏', '有什么问题能难倒无所不知的大机械头?'],
		mengqiuda: {
			audio: 6,
			logAudio: () => false,
			init(player) {
		...
		mengdayi: {
			...
			async content(event, trigger, player) {
				...
				if (links) {
					...
					let card;
					const { control } = await player.chooseControl('牌堆顶', '牌堆底').set('prompt', '从哪里开始检索？').forResult();
					if (control == '牌堆顶') {
		...
		hyyz_xt_sp_zhigengniao: ['知更鸟', ['female', 'hyyz_xt', 3, ['menghesong', 'mengxiezou'], []], '樾舟', ''],
		menghesong: {
			audio: 5,
			trigger: {
				global: 'useCardAfter'
			...
			async content(event, trigger, player) {
				const { bool } = await player.chooseUseTarget({
					...
					isCard: true
				}, false).set('logSkill', event.name).forResult();
				if (bool) {
		...
		meng_b3_sp_qingque: ['青雀', ['female', 'hyyz_xt', 3, ['mengmystouxian', 'mengmysmianzuo'], []], '落叶秋霜/尾改', ''],
		mengmystouxian: {
			audio: 'hyyzlaoyue',
			trigger: {
				player: 'phaseChange',
